...
def find_basic_solution(a, b, c, ready_sol = None):
    ...
    # Making the columns
    """
    temp_num = (num_of_variables - len(ready_sol)) if ready_sol is not None \
            else num_of_variables
            """
    columns = ['S_0']
    for i in range(1, num_of_variables + 1):
        if ready_sol is None or \
                ready_sol.get(str('x_' + str(i))) is None:
            columns.append('x_' + str(i))
    
    global needed_val
    needed_val = {}
    if ready_sol is not None:
        needed_val = ready_sol

    for i in columns[1:]:
        needed_val[i] = 0

    if ready_sol is not None:
        needed_val = dict(sorted(needed_val.items(), key=lambda x: x[0]))
...
def get_solution(table, opt):
    ...
    for i in range(len(table.values) - 1):
        if table.rows[i] in needed_val.keys():
            needed_val[table.rows[i]] = table.values[i][0]
...
def print_solution(F, solution, opt):
    ...
    min_or_max = 'min' if opt == 0 else 'max'

    print('{} value of F: {}'.format(min_or_max, F))

    for i in needed_val.keys():
        print(i, needed_val[i])

def simplex_internal(a, b, c, opt, ready_sol = None):
    # Method returns simplex table 
    c.insert(0, 0)
    table = find_basic_solution(a, b, c, ready_sol)
    if table:
