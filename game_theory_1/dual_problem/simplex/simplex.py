...
def find_basic_solution(a, b, c, num_of_variables):
...
def print_solution(F, solution, opt):
    ...
    for i in needed_val:
        print(i, solution[i])
        solut.append(solution[i])

    return solut


def simplex_internal(a, b, c, opt, num_of_variables):
    ...
    c.insert(0, 0)
    table = find_basic_solution(a, b, c, num_of_variables)
    optimize(table)
